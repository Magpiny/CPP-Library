...
/// by default output is only colorized if both stderr and stout are outputing
/// into terminal. when any of them is redirected to a file, colorization is
/// disabled.
...
template <class... Args>
std::string cformat(const std::locale &loc, std::string_view fmt,
                    Args &&...args) {
...
template <class... Args> void cprint(std::string_view fmt, Args &&...args) {
...
template <class... Args> void cprintln(std::string_view fmt, Args &&...args) {
